...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        let mut curr_height = self.height + 1;
        let mut prev_hash = hash(self);
        let mut res = true;
        for i in chain {
            if i.parent != prev_hash || i.height != curr_height {
                res = false;
                break;
            }
            prev_hash = hash(i);
...
fn build_valid_chain_length_5() -> Vec<Header> {
    ...
    chain.push(chain[0].child());
    chain.push(chain[1].child());
    chain.push(chain[2].child());
    chain.push(chain[3].child());
    chain
}
